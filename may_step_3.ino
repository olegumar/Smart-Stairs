...
// Параметры
const int SvLimit = 600;  // Переменная для сравнения с освещенностью.
int phDHT = 49;           // Переменная для сравнения с влажностью.
const int MinDist = 85
;   // Переменная для сравнения с шириной ступеньки, 80 см.
...
void setup()
{
  ...
  mySerial.begin(74880); //Инициализация своего последовательного порта на скорости 74880 бойд
...
void loop()
{
  ...
  if (Svet_Value < SvLimit)        // Сравнение "Достаточно ли темно для подсветки?".
  {
    ...
    if (sonicDOWN.read(CM) < MinDist) // Проверка факта срабатывания нижнего сенсора.
    {
      ...
      Serial.println("_Down_");
      Step(HIGH, LSBFIRST);        // Вызов процедуры на вкл лестницы сНизу вВерх.
      Start = MSBFIRST;            // Меняем направления, для выкл
    ...
    if (sonicUP.read(CM) < MinDist) // Проверка факта срабатывания верхнего сенсора.
    {
      ...
      Serial.println("_Up_");
      Step(HIGH, MSBFIRST);        // Вызов процедуры на вкл лестницы сВерху вНиз.
      Start = LSBFIRST;            // Меняем направления, для выкл
      delay(TimTrap);              // Задержка включенной лестницы.
    }
//   delay(200);                    // Задержка для чтения сенсоров.
    if (digitalRead(Dv_Pin) == LOW && Start != "null")  // Проверка факта срабатывания датчика движения.
    {
      Serial.println("_OFF_");
      LS = "OFF";
      Step(LOW, Start);            // Вызов процедуры на выкл лестницы.
...
void Step(int SIG, int Start)      // Процедура вкл или выкл подсветки
{
  for (int i = 0; i <= N; i++)
  {
    ...
    {
      if (sonicDOWN.read(CM) < MinDist || sonicUP.read(CM) < MinDist)
      {
        LS = "ON";
        return;                    // Если есть новое движение, выходим из процедуры выкл
      }
    }
  }
}
